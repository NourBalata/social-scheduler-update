      ...
      <div class="ap-modal" dir="ltr">
        ...
        <div class="ap-header">
          <div class="ap-header-left">
            ...
            <div>
              <h3 class="ap-title">AI Content Autopilot</h3>
              <p class="ap-subtitle">Full monthly content plan in one click</p>
            </div>
          </div>
        ...
        <div class="ap-body">
          ...
          <div v-if="step === 1" class="ap-step ap-step-form">
            ...
            <div class="ap-grid-2">
              <div>
                <label class="ap-label">Business Name *</label>
                <input v-model="form.business" class="ap-input" placeholder="e.g. Al-Asala Restaurant">
              </div>
              <div>
                <label class="ap-label">Industry / Sector *</label>
                <input v-model="form.industry" class="ap-input" placeholder="e.g. Restaurants, Fashion">
              </div>
              <div>
                <label class="ap-label">Target Audience *</label>
                <input v-model="form.audience" class="ap-input" placeholder="e.g. Young adults 18-35">
              </div>
              <div>
                <label class="ap-label">Content Goal *</label>
                <input v-model="form.goal" class="ap-input" placeholder="e.g. Increase sales, grow followers">
              </div>
              <div>
                <label class="ap-label">Page *</label>
              ...
              <div>
                <label class="ap-label">Posts per Week *</label>
            ...
            <!-- Tone selector -->
            <div>
              <label class="ap-label">Content Style *</label>
              <div class="ap-tone-grid">
                <button
                  ...
                  type="button"
                  class="ap-tone-btn"
                  :class="{ active: form.tone === t.value }"
          ...
          <div v-else-if="step === 2" class="ap-step ap-step-loading">
            ...
            <p class="ap-loading-title">AI is generating your plan...</p>
          ...
          <div v-else-if="step === 3" class="ap-step ap-step-preview">
            <div class="ap-summary">
              ✅ Generated <strong>{{ generatedPosts.length }}</strong> posts for page
              <strong>{{ form.page }}</strong> — review before scheduling.
            </div>
            ...
            <div class="ap-preview-actions">
              <button class="ap-btn-back" @click="step = 1">← Regenerate</button>
          ...
          <div v-else-if="step === 4" class="ap-step ap-step-success">
            ...
            <p class="ap-success-sub">All posts are now visible in the calendar below.</p>
            <button class="ap-btn-done" @click="close">Got it, view calendar</button>
          </div>
...
const props = defineProps({
  ...
  /** Array of page name strings passed from Blade */
  pages: {
    type: Array,
    default: () => [],
  },
  ...
  /** Route URLs (passed from Blade so we keep zero hardcoded URLs) */
  routes: {
    type: Object,
    default: () => ({
...
const successMsg   = ref('Your plan has been scheduled! 🎉')
const loadingMsg   = ref('Analyzing your business and crafting the right content')
...
function startProgressBar() {
  ...
  const interval = setInterval(() => {
    progress.value = Math.min(
      progress.value + (progress.value < 80 ? 2 : 0.3),
      92
    )
...
function close() {
  ...
  // small delay so user sees the close animation before reset
...
async function confirm() {
  ...
  try {
    const res = await fetch(props.routes.confirm, {
      ...
      body: JSON.stringify({
        page_name: form.page,
        posts:     generatedPosts.value,
      }),
    })

    const data = await res.json()
    successMsg.value = data.message ?? 'Your plan has been scheduled! 🎉'
    step.value = 4

    // Tell the parent page to refresh the calendar
    emit('scheduled', generatedPosts.value)

  } catch {
    // stay on step 3, let user retry
  } finally {
