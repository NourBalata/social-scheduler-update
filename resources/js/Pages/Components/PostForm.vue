...
// defineProps — البيانات اللي بتيجي من الـ parent (Laravel Blade)
...
// defineEmits — الأحداث اللي بنبعثها للـ parent
const emit = defineEmits(['open-page-modal', 'open-media-library'])

// ref() — متغيرات تفاعلية، لما تتغير الـ UI يتحدث تلقائياً
...
// دالة مساعدة لحساب الوقت الافتراضي
...
function fillBestTime() {
    const pad = n => String(n).padStart(2, '0')
    const now = new Date()
    scheduledAt.value = `${now.getFullYear()}-${pad(now.getMonth()+1)}-${pad(now.getDate())}T18:00`
...
function toggleHashtag(tag) {
    if (selectedHashtags.value.includes(tag)) {
        ...
        content.value = content.value.replace(' ' + tag, '').replace(tag, '').trim()
    } else {
        selectedHashtags.value.push(tag)
        content.value += (content.value && !content.value.endsWith(' ') ? ' ' : '') + tag + ' '
    }
}

async function magicWrite() {
    if (content.value.trim().length < 5) {
        showToast('💡 Type a quick idea first!')
        return
    }
    aiLoading.value = true
    try {
        const res = await fetch(props.aiRoute, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': props.csrf },
            credentials: 'same-origin',
            body: JSON.stringify({ idea: content.value.trim() }),
        })
        ...
        if (data.captions) {
            ...
            const timer = setInterval(() => {
                if (i < full.length) content.value += full[i++]
...
function saveDraft() {
    if (!content.value.trim()) { showToast('⚠️ Write something first!'); return }
    localStorage.setItem('postflow_draft', content.value)
