...
import DateClickModal from './components/DateClickModal.jsx';
...
const vueApp = createApp({
    setup() {
        ...
        return { autopilotRef, dateClickRef };
    }
});
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (postForm) {
        ...
        function updateCharCount() {
            ...
            if (remaining < 500) charCountDiv.classList.add('warning');
            if (remaining < 100) charCountDiv.classList.add('error');
        ...
        function updatePreviewTime() {
            ...
            if (scheduledDate) {
                ...
                if (diff > 0) {
                    ...
                    if (days > 0)       previewTime.textContent = `Scheduled for ${days} day${days > 1 ? 's' : ''} from now`;
                    else if (hours > 0) previewTime.textContent = `Scheduled for ${hours} hour${hours > 1 ? 's' : ''} from now`;
                    else                previewTime.textContent = 'Scheduled soon';
                } else {
        ...
        function updatePreviewMedia() {
            ...
            if (mediaUrl && mediaType === 'image') {
                ...
                previewImage.onerror = () => { previewMedia.style.display = 'none'; };
            } else {
        ...
        function validateMediaUrl() {
            const mediaUrl = mediaUrlInput.value.trim();
            if (mediaUrl) {
                try   { new URL(mediaUrl); mediaUrlInput.setCustomValidity(''); }
                catch { mediaUrlInput.setCustomValidity('Please enter a valid URL'); }
            } else {
        ...
        function detectMediaType() {
            ...
            if (url) {
                if (url.match(/\.(jpg|jpeg|png|gif|webp)$/)) mediaTypeSelect.value = 'image';
                else if (url.match(/\.(mp4|mov|avi|webm)$/)) mediaTypeSelect.value = 'video';
                updatePreviewMedia();
            }
        }

        contentArea.addEventListener('input', () => { updateCharCount(); updatePreviewContent(); });
        pageSelect.addEventListener('change', updatePreviewPage);
        mediaUrlInput.addEventListener('input', () => { validateMediaUrl(); detectMediaType(); });
        ...
        postForm.addEventListener('submit', function (e) {
            const content = contentArea.value.trim();
            if (content.length === 0) {
                e.preventDefault(); alert('Please enter post content'); contentArea.focus(); return false;
            }
            if (content.length > 5000) {
                e.preventDefault(); alert('Post content is too long (max 5000 characters)'); contentArea.focus(); return false;
            }
                    ...
                    data.forEach(post => {
                        ...
                        if (row) {
                            ...
                            if (statusCell && statusCell.textContent !== post.status) {
                                ...
                                setTimeout(() => { statusCell.style.background = ''; }, 1000);
                            }
                        }
                    });
                })
    ...
    aiBtn?.addEventListener('click', async () => {
        ...
        try {
            const res  = await fetch('/ai/caption', {
                method:  'POST',
