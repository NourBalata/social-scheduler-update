...
async function generateSallaCaption() {
    ...
    btn.disabled = true;
    btn.innerHTML = '<div style="width:16px;height:16px;border:2px solid rgba(255,255,255,.4);border-top-color:#fff;border-radius:50%;animation:spin 1s linear infinite;display:inline-block;vertical-align:middle;margin-right:6px;"></div> Generating...';
    ...
    try {
        ...
        // نفس UX الـ post — نعرض 3 خيارات يختار منها المستخدم
        const captions = data.captions ?? (data.caption ? [data.caption] : []);
        if (!captions.length) { alert('⚠️ No captions returned. Please try again.'); return; }
        ...
        btn.disabled = false;
        btn.innerHTML = '<svg width="16" height="16" fill="none" stroke="white" stroke-width="2" viewBox="0 0 24 24" style="vertical-align:middle;margin-right:6px;"><path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg> Generate AI Caption';
    }
}
...
function showSallaCaptionPicker(captions) {
    ...
    captions.forEach((caption, i) => {
        ...
        btn.onclick = () => {
            // typewriter effect — نفس الـ post
            textarea.value = ''; let j = 0;
