            ...
            @foreach($plans as $plan)
                <div class="plan {{ $currentPlan && $currentPlan->id === $plan->id ? 'active' : '' }}">
                    @if($currentPlan && $currentPlan->id === $plan->id)
                        <span class="badge">{{__('Current Plan')}}</span>
                    @endif
                    ...
                    <ul>
                        <li>{{ $plan->posts_limit }} {{ __('posts per month')}}</li>
                        <li>{{ $plan->pages_limit }} {{ __('Facebook')}} {{ $plan->pages_limit > 1 ? 'pages' : 'page' }}</li>
                        <li>{{ __('Post scheduling')}}</li>
                        @if(!$plan->isFree())
                    ...
                    @if($currentPlan && $currentPlan->id === $plan->id)
                        ...
                        @else
                            <button disabled>{{__('Current Plan')}}</button>
                        @endif
                    @else
