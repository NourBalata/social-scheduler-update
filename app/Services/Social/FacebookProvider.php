...
class FacebookProvider implements SocialMediaProvider
{
    ...
    {
        $response = Http::get("{$this->graphUrl}/{$pageId}/insights", [
            'access_token' => $token,
            'metric'       => $metric,
            'period'       => $period,
        ]);
        ...
        if ($response->failed()) {
            $message = $response->json('error.message') ?? $response->body();
            Log::error('Facebook: getPageInsights failed.', ['page_id' => $pageId, 'error' => $message]);
            throw new \Exception("Facebook API Error: {$message}");
        }
    ...
    public function getPostInsights(string $token, string $postId): array
    {
        ...
        $response = Http::get("{$this->graphUrl}/{$postId}/insights", [
            'access_token' => $token,
            'metric'       => $metrics,
        ]);
        ...
        if ($response->failed()) {
            $message = $response->json('error.message') ?? $response->body();
            Log::error('Facebook: getPostInsights failed.', ['post_id' => $postId, 'error' => $message]);
            throw new \Exception("Facebook API Error: {$message}");
        }
