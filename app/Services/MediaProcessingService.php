...
class MediaProcessingService
{
    ...
    public function upload(UploadedFile $file, int $userId, ?int $folderId = null): Media
    {
        ...
        if ($type === 'image') {
            // الصور سريعة — نعملها synchronous
            [$width, $height, $thumbPath] = $this->processImage($path, $userId);
        } elseif ($type === 'video') {
            // الفيديو بطيء — نحفظ الـ record أولاً، والـ thumbnail بيجي بعدين via job
            $meta['size_original'] = $file->getSize();
            // نحاول نجيب الـ metadata بسرعة بدون thumbnail
            try {
                ...
                // ما في FFmpeg أو الملف غير صالح — ما بيمنع الـ upload
            }
        }
        ...
        // الـ thumbnail للفيديو: نعمله async بعد ما نحفظ الـ record
        if ($type === 'video') {
            \App\Jobs\ProcessVideoJob::dispatch($media, 'thumbnail', [])->onQueue('video');
        }
    ...
    // Public wrapper يُستدعى من ProcessVideoJob بعد الـ upload
    public function generateVideoThumbnail(Media $media): void
    {
        try {
            [,$thumbPath, $width, $height] = $this->processVideo($media->path, $media->user_id);
            ...
            \Illuminate\Support\Facades\Log::warning("Thumbnail generation failed for media #{$media->id}: " . $e->getMessage());
        }
    }
