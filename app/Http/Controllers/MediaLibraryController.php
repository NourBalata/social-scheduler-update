...
class MediaController extends Controller
{
    ...
    public function compressVideo(CompressVideoRequest $request, Media $media): JsonResponse
    {
        ...
        // بدل ما ننتظر FFmpeg (ممكن دقائق)، نحط الـ job على queue ونرجع فوراً
        \App\Jobs\ProcessVideoJob::dispatch($media, 'compress', $request->validated());

        return response()->json([
            'message' => 'Video compression queued. You will see the result shortly.',
            'data'    => new MediaResource($media),
        ], 202);
    }
    ...
    public function trimVideo(Request $request, Media $media): JsonResponse
    {
        ...
        \App\Jobs\ProcessVideoJob::dispatch($media, 'trim', [
            'start' => (float) $request->start,
            'end'   => (float) $request->end,
        ]);

        return response()->json([
            'message' => 'Video trim queued. You will see the result shortly.',
            'data'    => new MediaResource($media),
        ], 202);
    }
